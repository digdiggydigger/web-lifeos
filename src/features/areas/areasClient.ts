...
import type { Capture, LifeArea, Log, Task, TaskStatus } from '@/domain/types';
...
export interface LifeAreaDetailClient {
  ...
  /** Every live capture; the detail keeps only the ones filed to its area. */
...
export function firebaseAreaClients(
  ...
  return {
    ...
    fetchAllCaptures: async () => (await captures.fetchCaptures(db, uid)).items.slice(),
