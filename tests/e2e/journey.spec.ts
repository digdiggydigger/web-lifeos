...
test.describe('Phase 2', () => {
  ...
  test('a nudge done for now, a sprint from Today, inbox zero held behind the sheet, and the daily summary', async ({
    page,
  }) => {
    const email = await signUpAndEnter(page, 'phase2');
    const layer = page.getByTestId('celebration-layer');
    await seedDueNudge(await uidFor(email), 'Stretch');
    await page.reload();

    // Nudges: the due card on Today, done for now.
    const nudge = page.getByRole('article', { name: 'Stretch, due' });
    await nudge.getByRole('button', { name: 'Dismiss Stretch' }).click();
    await expect(page.getByRole('article', { name: 'Stretch, due' })).toHaveCount(0);

    // Focus: a sprint started from Today runs in the bar on every tab, then stops on request.
    await page
    ...
    // Celebrations: emptying the inbox from the Create Task sheet is held until the sheet has
    // closed, then plays full screen over the empty inbox.
    await nav(page, 'Captures');
    await page.getByRole('button', { name: 'Capture something' }).first().click();
    const composer = page.getByRole('dialog');
    await composer.getByPlaceholder("What's on your mind?").fill('Book the dentist');
    ...
    await expect(layer).toHaveAttribute('data-last-burst', 'milestone');
    await expect(layer).toHaveAttribute('data-celebrating', 'true');

    // The daily summary, synthesised on-device against the emulators.
    await nav(page, 'Today');
    await page
      .getByRole('link', { name: /week review/i })
      .first()
      .click();
    ...
    await expect(
      summary
        .getByRole('article', { name: 'Generated summary' })
        .getByLabel('Generated by On-device synthesis, Energizing tone'),
    ).toBeVisible();
  });
});
